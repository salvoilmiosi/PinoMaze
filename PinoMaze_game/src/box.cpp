...
void box::setSize(float w, float h, float l, float uv) {
    width = w;
    height = h;
    length = l;
    texSize = uv;
...
bool box::init() {
    ...
    if (!createBuffers(vertices, indices, 24, 36)) return false;
    if (!calculateNormals(vertices, indices, 24, 36)) return false;
    if (!calculateTangents(vertices, indices, 24, 36)) return false;
...
void box::render() {
    if (!matrices.empty()) {
        bindModel();
        glDrawElementsInstanced(GL_TRIANGLES, 36, GL_UNSIGNED_INT, 0, (GLsizei)matrices.size());
        unbindModel();
    }
}
