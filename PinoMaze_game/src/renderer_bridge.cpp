...
bool bridgeRenderer::init() {
	...
	return checkGlError("Failed to init bridge model");
}
...
void bridgeRenderer::drawArcWall(float z, float w, float texSize) {
    ...
    for (int i=0; i<bridgeSubsV; ++i) {
        ...
        for (int j=0; j<=bridgeSubsH; ++j) {
            ...
            v1.u = - angle_under * bridgeWallHeight / bridgeArcHeight / M_PI / texSize;
            v2.u = - angle_over * bridgeWallHeight / bridgeArcHeight / M_PI / texSize;
